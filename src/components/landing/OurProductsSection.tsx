  ...
  return (
    <section className="pt-20 pb-12 bg-[#f5f5f5] dark:bg-[#09090b] transition-colors duration-200">
      <div className="max-w-[1800px] mx-auto px-4 sm:px-8 md:px-12">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end mb-14 gap-8">
          <div className="max-w-3xl">
            <motion.div
              ...
              viewport={{ once: true }}
              className="inline-flex items-center rounded-full border border-zinc-200 dark:border-white/10 bg-white dark:bg-zinc-900 px-3 py-1 text-[11px] font-bold uppercase tracking-wider text-zinc-900 dark:text-zinc-200 mb-6 shadow-2xs"
            >
              OUR PRODUCTS
            </motion.div>
          ...
          >
            ...
            <Link href="/products">
              <button className="bg-[#1c1c1c] dark:bg-zinc-800 hover:bg-black dark:hover:bg-zinc-700 text-white border border-transparent dark:border-white/10 px-5 py-2.5 rounded-full font-semibold text-sm flex items-center gap-2 transition-colors cursor-pointer shadow-xs">
                View All Products
        ...
        {/* Product Cards Grid (Matching Exact Reference UI: 3-column layout) */}
          ...
          {ourProducts.map((product, index) => {
            const cardInner = (
              <motion.div
                ...
                className="group relative h-full bg-white dark:bg-zinc-900/90 rounded-2xl p-7 sm:p-8 flex flex-col justify-between border border-zinc-200/80 dark:border-white/10 hover:border-blue-400/50 dark:hover:border-blue-400/30 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 cursor-pointer min-h-[280px]"
              >
                {/* Top Area: Logo, Title, Description */}
                <div>
                  {/* Product Logo - Clean without border or background container */}
                  <div className="h-14 w-14 mb-6 group-hover:scale-105 transition-transform duration-300 flex items-center">
                    <Image
                      src={product.logo}
                      alt={product.name}
                      width={56}
                      height={56}
                      className="w-auto h-full max-h-14 max-w-14 object-contain rounded-md"
                    />
                  </div>

                  {/* Product Title */}
                  <h3 className="text-2xl font-bold text-zinc-900 dark:text-white mb-2.5 tracking-tight group-hover:text-[#0066cc] dark:group-hover:text-blue-400 transition-colors duration-300">
                    {product.name}
                  </h3>

                  {/* Product Description */}
                  <p className="text-[14.5px] text-zinc-600 dark:text-zinc-400 font-normal leading-relaxed mb-6">
                    {product.description}
                  </p>
                </div>
                ...
                <div className="pt-2">
                  <span className="inline-flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-[#0066cc] dark:text-[#38bdf8] group-hover:gap-2.5 transition-all duration-200">
                    TRY NOW
